...
let arrowSpeed = 100;
let arrowBoneTween, featherTween;
...
function shootArrow(event) {
  ...
  // Guardar posición y lookAt para restaurar después
  ...
  // Iniciar tweens de animación de huesos y plumas
  initArrowTweens(arrowContainer);

  // Seguir flecha con cámara
  followArrow = arrowContainer;

  // Slow motion si está activo
  if (params.slowMotionActive) {
    normalTimeStep = (1 / 60) * slowMotionFactor;
  }
}
...
async function stickArrow(arrowObj, body, hitPoint) {
  ...
  // 1. Detener tweens
  if (arrowBoneTween) arrowBoneTween.stop();
  if (featherTween) featherTween.stop();

  // 2. Eliminar rigidBody y desactivar física
  physicsWorld.removeRigidBody(body);
  body.setActivationState(Ammo.DISABLE_SIMULATION);
  arrowObj.userData.physicsBody = null;

  // 3. Restaurar forma original de la flecha
  originalFormArrow(arrowObj);

  // 4. Offset fijo para que la PUNTA quede exactamente en el hitPoint
  //    Cambia 12 por el valor exacto que necesites
  const arrowOffset = new THREE.Vector3(-19, 0, 0).applyQuaternion(
    arrowObj.quaternion
  );

  // Posición final del grupo de la flecha
  const finalPos = hitPoint.clone().add(arrowOffset);

  arrowObj.position.copy(finalPos);

  // Mantener la rotación de la flecha justo antes del impacto
  arrowObj.quaternion.copy(currentArrowObject.quaternion);

  // 5. Limpiar variables globales para permitir nuevo disparo
  currentArrowBody = null;
  currentArrowObject = null;
  followArrow = null;

  // 6. Esperar para ver la flecha clavada
  await new Promise((resolve) => setTimeout(resolve, 3000));

  // 7. Restaurar control de cámara
  canShoot = true;
  camera.position.copy(cameraPreviousPosition);
  camera.lookAt(cameraPreviousLookAt);

  // 8. Restaurar timestep normal
  normalTimeStep = 1 / 60;
}
...
function animateArrow(time) {
  ...
  const amplitude = 0.2; // máximo giro en radianes
...
function initArrowTweens(arrowObj) {
  if (!arrowObj) return;
  ...
  const amplitude = 0.2;
  const featherAmp = 0.8;

  // Tween para huesos de la flecha
  const waveState = { phase: 0 };
  arrowBoneTween = new TWEEN.Tween(waveState)
    .to({ phase: Math.PI * 2 }, 2000) // 2 segundos por ciclo
    .onUpdate(() => {
      const p = waveState.phase;
      for (let i = 1; i < n - 1; i++) {
        const t = i / (n - 1);
        const centerFactor = Math.sin(Math.PI * t);
        bones[i].position.z = Math.sin(p) * amplitude * centerFactor;
      }
      bones.forEach((b) => b.updateMatrixWorld());
    })
    .repeat(Infinity)
    .start();
  ...
  if (featherGroup) {
    const feathersState = { phase: 0 };
    featherTween = new TWEEN.Tween(feathersState)
      .to({ phase: Math.PI * 2 }, 2000)
      .onUpdate(() => {
        const wobble = Math.sin(feathersState.phase) * featherAmp;
        featherGroup.children.forEach((feather, i) => {
          if (i === 0 || i === 2) feather.rotation.x = wobble;
        });
      })
      .repeat(Infinity)
      .start();
